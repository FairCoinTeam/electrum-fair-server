...
"""
    Class description: Set that remembers original insertion order
...
class OrderedSet(collections.MutableSet):
    ...
    def discard(self, key):
        if key in self.map:
            key, prev_element, next_element = self.map.pop(key)
            prev_element[2] = next_element
            next_element[1] = prev_element
